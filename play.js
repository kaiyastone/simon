...
class Game {
    ...
    constructor() {
        ...
        document.querySelectorAll('.game-button').forEach((el, i) => {
            if (i < btnDescriptions.length) {
                this.buttons.set(el.id, new this.buttons(btnDescriptions[i], el));
            }
        });
        
        const playerNameE1 = document.querySelector('.player-name');
        playerNameE1.textConent = this.getPlayerName();
    }
    ...
    async pressButton(button) {
      ...
      saveScore(score) {
        ...
        scores = this.updateScores(userName, socre, scores);
